 ...
 *  优点：消掉了哪些具体问题，点名到机会点编号（编号+一句话复述内容，禁止只写编号）；禁止"更优雅""更好维护"这类空话
 ...
 *  未被任何候选方案覆盖的机会点，必须列入"本次范围外"（编号+一句话复述）并写明原因，不得静默丢弃
...
引用机会点编号（O1、O2…）时，编号后必须紧跟该机会点的一句话内容（如"O3：重试逻辑分散在两层"），任何一处引用都禁止只写编号。  
...
## 候选方案
[2-3个自洽整包，每个含：一句话主张 / 优点（点名机会点编号+一句话复述）/ 缺点 / 影响面粗估；并写明区分方案的取舍轴]
