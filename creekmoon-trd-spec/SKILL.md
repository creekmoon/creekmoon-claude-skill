...
version: 1.2.0
description: 技术实现文档（TRD）写作规范，用于编写或迭代技术方案，并在初版 TRD 中原生包含“最终落地实现方案”。Make sure to use this skill whenever the user asks for 技术实现文档、TRD、技术方案、落地方案、最终实现方案、方案收束、实现决策说明，尤其适用于需要说明现状切入点、模块边界、接口契约、最终决策、代码架构与实现细节、风险实际影响和最小修改范围的场景。
---
...
- **2026-05-15**：新增"现状与切入点"章节规范，区分新增需求与修改型需求，强化开发者业务心智模型建立。
- **2026-05-15**：新增"最终落地实现方案"章节规范，把最终决策、代码架构/实现细节、风险实际影响和最小修改范围前置进 TRD 初版。
...
- **3张表（必填）+ 1个落地摘要（修改型/落地型需求建议保留）**：
    ...
    - **落地修改摘要**：最终改什么、不改什么、为什么不扩散（≤5行；完整内容放第6节）
...
- [ ] 已写"最终落地实现方案"，说明最终决策、修改清单、风险复核和不改范围
...
- **复杂修改型 TRD 可放宽到 280 行左右**：前提是新增内容集中在现状切入点、风险复核和最终落地方案
