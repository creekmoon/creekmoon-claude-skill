...
version: 2.2.0
description: 为代码仓库生成或重构根目录 README 的项目总览技能。只要用户提到 README、项目介绍、项目文档、架构说明、新人接手、仓库导览、模块边界、模块协作，就应该优先使用本技能。特别适合把零散工程结构整理成“读者 3 分钟能建立心智模型”的根 README：准确说明项目是什么、包含哪些功能模块、核心业务流程怎么走；当目标读者明显是新同学时，还应补一段轻量的代码阅读地图，帮助读者知道先从哪几个目录、模块或关键入口文件入手建立文件模型。生成时默认从代码证据出发，代码细节只作为理解功能边界和阅读路径的线索，不把 README 写成业务说明书、接口文档、代码说明书或深度研究报告。
---
...
前半部分至少包含：标题与一句话定位、一屏摘要、业务背景与主线。后半部分至少包含：全局架构与新人入门、功能模块清单、核心业务流程。
...
- 用户希望 README 顺手告诉新同学“先从哪几个代码位置看起”
...
- 默认只写功能层与模块边界；只有在 `全局架构与新人入门` 一节里，才允许点到为止地引用少量目录、模块或关键入口文件，帮助新人建立文件模型
- 新人导读只回答“先看哪里、为什么、下一步看哪里”，不展开包名清单、类方法细节、SQL、字段校验、装配细节
...
- 新人导读默认控制在 `3-5` 个阅读入口内，优先给“最小可行阅读路径”，不要写成完整接手手册
...
- 如果用户明显面向新人接手，再额外判断是否需要补 `全局架构与新人入门`；它属于技术篇补充，不替代业务主轴
...
| 3 | 新人阅读入口 | 工作区结构、模块依赖关系、启动模块、主链路入口、少量关键文件 |
...
- 如果用户明确面向新人接手，额外找出 `3-5` 个最能建立文件模型的阅读入口；优先目录、模块 README、启动模块、主链路入口，不优先底层工具类
...
2. **边界审查**
    ...
    - 我给的是帮助新人建立文件模型的最小阅读路径，还是已经写成了开发手册？
...
4. **叙事顺序审查**
    ...
    - 新人导读应该放在技术篇开头，承接业务主线，不要挤进业务篇里破坏叙事重心
...
**技术/工程篇（后半部分）**：先给轻量的全局架构与新人阅读地图，再讲功能边界与流程，帮助建立工程心智模型。
...
这一节面向第一次接手项目的人。目标不是讲清全部实现，而是给出一条**最小可行阅读路径**，让读者把前面的项目模型映射到代码里的目录、模块和关键入口上。
...
推荐格式：轻量表或 `3-5` 条短列表，回答“先看哪里、为什么先看、看完知道什么、下一步看哪里”。

推荐表格：

| 先看哪里 | 为什么先看 | 看完会建立什么认知 | 下一步看哪里 |
|----------|------------|--------------------|--------------|

写法要求：

- 优先写目录、子模块、模块 README、工作区配置、启动模块、主链路入口；必要时才点到少量关键入口文件
- 默认控制在 `3-5` 个阅读入口，不做穷举
- 每一行只解释“为什么值得先看”和“看完知道什么”，不要展开实现细节
- 允许出现少量目录名、模块名、关键入口文件名；默认不出现类名、方法名、SQL、配置 key
- 不写启动命令、联调步骤、环境依赖、排障手册
- 读者看完这一节，应该知道“接下来先看哪里”，而不是被迫一次性理解全部工程细节
...
- **禁止**出现代码层面的列（包名、类名、接手入口、改代码时先看哪里）；这些信息只能放在上一节的轻量新人导读里
...
- **保留原有有价值内容**
- **优先前置新增“项目总览”章节**
- 如果用户明确面向新人接手，可在技术篇开头新增 `全局架构与新人入门`，但不要把原有开发手册整段搬进 README
...
- 新人导读表默认 `3-5` 行、`4` 列以内，够读者建立阅读路径即可
...
- 可以点名模块名；只有在 `全局架构与新人入门` 中，才允许少量点名目录或关键入口文件，但不要展开类名、方法、SQL、字段校验、配置 key
...
- 新人导读解释的是阅读顺序，不是操作步骤：写“先看哪里”，不写“怎么启动 / 怎么联调 / 怎么排障”
- 功能模块说明应该靠前；如果存在新人导读，应放在技术篇开头；目录树、启动命令、接口地址、包结构图等操作清单不纳入根目录 README
...
- 把新人导读写成完整开发手册，塞满启动命令、环境依赖、联调步骤
...
- 表格简洁
- 以“先业务后技术”为默认定位：先定位与摘要，再业务背景与主线，然后全局架构与新人入门，再功能模块清单，最后核心业务流程
