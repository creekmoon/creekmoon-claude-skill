...
- README 先回答“这个仓库是什么、直接拥有哪部分能力”，再回答“包含哪些功能模块、核心业务流程怎么走”；**不展开**代码结构、启动命令、配置项、调试入口等操作性细节
- README 是**项目级顶层视野**，不是代码说明书，也不是详细设计文档；代码包结构、对象流转、生命周期、工程操作手册等深度内容应留给二次提问或专门文档
- 优先讲 **功能定位与模块边界**，代码细节只作为理解功能边界的线索
- 在 `### 1` 和 `### 2` 里，必须先建立读者心智模型：项目定位、项目类型、功能模块、核心流程
...
4. **叙事顺序审查**
    - `### 1` 和 `### 2` 的首句应该先让读者知道“项目职责、项目类型、功能模块、核心流程”
    - 如果目前只能列目录，还说不清模块边界和核心流程，先回去补判断，不要直接落正文
...
默认按下面顺序组织 README。除非项目类型明显不适配，否则不要轻易跳过前 4 节。
...
- 项目名称
- 1 句话说明它在系统中承担什么职责：服务平台、业务系统、能力中心、适配网关、聚合工程等
- 1 句话说明它直接拥有的能力和不直接拥有的外部能力
- 极短技术栈摘要（可选，且不能盖过项目定位）
...
这一节是整个 README 的核心入口，必须做到“任何人快速扫完就知道这个项目是做什么的”。

建议包含：

- **一句话业务模型**：写成 `输入/触发 -> 核心处理 -> 输出/结果`
- **1 张极简项目结构图**（可选）：只画 3-5 个功能模块节点，不画所有分支
- **1 张项目事实表**
...
| 项 | 说明 |
|----|------|
| 项目类型 | 业务系统 / 服务平台 / SDK / 能力中心 / 聚合工程 |
| 主要场景 | `1-3` 个核心使用场景 |
| 功能模块 | `2-4` 个关键模块及一句话职责 |
| 外部边界 | 本仓库直接依赖的 `1-3` 个外部系统或协议 |

事实表里**不出现**技术细节列（项目类型、主要入口、包域、关键依赖）；它们不属于项目级顶层视野。
...
只有当模块边界容易混淆，或者功能清单里一行还说不清分工时才补这一节。
...
- 先结论，后展开
- `### 1` 和 `### 2` 的首句优先写项目职责、功能模块和核心流程，不要先写业务愿景或技术栈清单
- 段落短，句子直
...
- 表格简洁
- 以“项目级顶层视野”为默认定位：先定位与摘要，再功能模块清单，最后核心业务流程
