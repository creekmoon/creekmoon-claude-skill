...
| `creekmoon-topaz-pm` | 产品经理 + 产品设计角色技能：判断功能取舍与优先级、补全场景边界，并直接产出页面结构设计（轨道、页面形态、布局骨架、信息分区、状态清单、灰阶线框原型），是架构/设计/开发链路的结构源头；必要时独立产出完整 PRD | 做一个XX模块/页面、评估方向与取舍、排优先级、定页面结构、出线框原型 |
...
| `creekmoon-trailblazer-readme` | 按场景生成或重构根目录 README：业务项目走总览+接手框架，工具类项目走用法优先（安装/启动/配置） | 业务接手 README、CLI/MCP/SDK 用法 README、重构根文档 |
| `creekmoon-apidoc-spec` | 标准化接口文档规范，统一概述、版本、环境、请求/响应、示例结构                     | 对外接口文档、开放平台文档、资源接口说明 |
| `creekmoon-aglaea-design` | 视觉与质感增强技能（锦上添花层）：视觉方向与设计令牌（配色/字体/间距/圆角/阴影）、组件质感、动效与反 AI 模板打磨；消费上游结构产物，不重新决定产品结构 | 打磨页面质感、去 AI 味、定配色与设计规范、加交互动效、实现页面时注入视觉质量 |
| `creekmoon-himeko-auto-decision` | 长线任务的自动决策引擎，按激进/均衡/保守模式扫描产品文档、量化漂移并全权裁决下一步，不抛问题给人 | 多轮迭代开发、指定决策模式、需要 AI 自主排优先级与控范围蔓延 |
| `creekmoon-strategic-decision-report` | 技术战略决策报告写作与核查标准——向领导汇报已定技术方案，讲清"要怎么实施、考虑过哪些取舍、最终怎么定"，写后派独立子代理以读者视角核查 | 向领导汇报技术决策、写决策汇报材料、把已定方案提炼成给领导看的报告 |
...
3. 如果想从产品经理视角把项目/需求想清楚（判断要不要做、先做什么、怎么改进），或要为新功能/页面定结构，用 `creekmoon-topaz-pm`：它默认产出 PM 判断 + 结构蓝图（页面形态、布局骨架、信息分区、状态清单、灰阶线框、验收标准），产物可直接支撑架构、设计、开发三个阶段，必要时也直接产出完整 PRD。它与偏业务沟通文风的 `creekmoon-prd-spec` 面向不同场景，二者相互独立，按需各自选用。
4. 如果是代码相关任务，通常建议组合使用：

- 先用 `creekmoon-lightcone-memory` 建立项目上下文
- 再用 `creekmoon-code-style` 约束具体实现风格

5. 如果要提升页面的视觉质感，用 `creekmoon-aglaea-design`：它是锦上添花的增强层——在上游（`creekmoon-topaz-pm` 或你自己）已定的页面结构之上，确定视觉方向和设计令牌，按组件规则和动效规范注入质感，并用反 AI 模板清单自检。不启用它不影响功能与表达的正确性，只影响好不好看。完整页面链路即"topaz 定结构 → aglaea 上质感"。
