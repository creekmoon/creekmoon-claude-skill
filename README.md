...
| `n8n-artist` | N8N艺术家——n8n 工作流编排美学与健康管理：画布布局整理与节点语义命名、Code 节点 JS/Python 优雅风格、执行历史与失效变量引用异常巡检、Agent 工具编排设计（调用链顺序与兜底、toolDescription、$fromAI 参数质量、检索覆盖缝隙）、LLM 节点提示词瘦身（保守改动+预览确认）、工作流四维评分（命名/参数/历史运行/业务编排各 10 分）；支持 n8n MCP 自动拉取工作流与执行历史，或粘贴 JSON 兜底 | 检查/体检 n8n 工作流、给工作流打分、整理编排布局、规范节点命名、排查执行失败与失效引用、设计 Agent 工具调用、精简 AI 节点提示词 |
