...
- **引用不替代内容**: 日期、需求单号、负责人、PR 链接只是边缘化的定位元数据；关键信息（问题、原因、处理方式）必须自包含在注释里，禁止 `/* 详见 PROJ-1234 */`、`/* 2026-09-14 修复 */` 这类只有引用没有内容的空壳注释
...
- 针对**已有业务**做修复、兼容分支、业务特例分支、临时旁路、灰度补丁时，新代码部分的块注释建议以日期为前缀
- 格式就一句话：`/* yyyy-MM-dd 现在做什么，解决什么问题 */`，问题与动作的顺序不限；不需要类型标签，不堆砌机制细节
- 精准用词、简洁第一：一句话能说清就不写两句；下线时机等补充信息仅在真正需要时用半句带上，不是必填
- 日期前缀只是辅助定位，不能只有日期没有内容
- 新业务的正常主线开发不加日期前缀——主线代码不是补丁
- 正面: `/* 2026-09-14 自清理过期标签，防止同步时一次读入撑爆堆 */`
- 正面: `/* 2026-09-14 之前一次读入全量标签会 OOM，现在前置一步自清理 */`
- 反面: `/* 2026-09-14 修复: 标签日志只增不删，同步时一次读入会撑爆堆；拼快照只要每个标签最新一条，写路径自清后可评估下线 */` — 类型标签冗余、细节堆砌，一句话的事写成流水账
- 反面: `/* 2026-09-14 修复 */` — 只有日期没有内容，读者必须翻提交记录才能理解
- 反面: `/* 兼容老数据 */` — 无日期，且"兼容"太泛：什么数据、怎么处理都没说
