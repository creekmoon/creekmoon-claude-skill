...
```java
/* 2026-09-14 兼容: 2026-09 前的老订单没有 carrierCode，回退到物流单取值；老数据补齐后本分支可下线 */
if (physicalCarrierCode == null) {
    physicalCarrierCode = logistics.getPhysicalCarrierCode();
}
...
格式：`/* yyyy-MM-dd 类型: 原因与背景 */`，类型为 修复/兼容/特例/临时/灰度 之一。日期前缀只是辅助定位，注释主体必须自包含关键信息（见「引用不替代内容」）。

为什么：

- 日期让后续读者一眼判断补丁"年龄"，评估特例分支是否可以下线
- 配合 git blame 快速回溯引入时的背景与上下文
- 无日期的补丁注释回答不了"这段特例为什么存在、还要留多久"
