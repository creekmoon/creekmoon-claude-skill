...
注释存在的唯一理由是提供代码本身读不出来的信息。代码能表达清楚的，不写注释；表达不清时，先改代码（命名、结构），再考虑注释。

R9 分区注释、R12 方法 Javadoc、R7 Stream 步骤注释是本原则在各场景的实例，格式与细则按各规则执行。本规则管更上层的三个判断：该不该写、写什么、写完后怎么维护。

### 先代码后注释

注释是表达失败后的补偿手段。当读者必须依赖注释才能看懂一段代码时，第一动作是改命名或调结构，第二动作才是写注释。

反面（用注释弥补烂命名）：

```java
Map<String, Integer> m; // key是承运商编码，value是可派件数
```

正面（改名后注释自然消失）：

```java
Map<String, Integer> carrierCode2DispatchableQty;
```
...
- **为什么这么做**——做了什么代码自己会说，不需要复述
- **业务背景与外部约束**——依赖的下游行为、历史数据兼容、上游约定
- **坑与反直觉点**——顺序敏感、隐式前提、曾经踩过的坑
- **为什么不走另一条明显路径**——防止后来者把它"优化"回错误写法
...
dispatchMQ.send(order.getId());
```

正面（解释顺序约束，有信息增量）：

```java
/* 必须先落库再发 MQ：消费者收到消息会立刻查库，顺序反了会查到旧状态 */
order.setStatus(DISPATCHED);
orderMapper.updateById(order);
dispatchMQ.send(order.getId());
```
...
日期、需求单号、负责人、PR/commit 链接等引用只是边缘化的定位元数据——它们帮助回溯，但本身信息量低，回答不了"这段代码为什么长这样"。关键信息（问题是什么、为什么这样处理、何时可下线）必须自包含在注释里，读者不该被迫跳到提交记录或需求系统才能理解。
...
正面（引用保留，但内容自包含——自包含不等于长，一句话即可）：

```java
/* 2026-09-14 老订单无 carrierCode，回退到物流单取值 */
if (physicalCarrierCode == null) {
    physicalCarrierCode = logistics.getPhysicalCarrierCode();
}

/* TODO 2026-09-14 张三: 等下游支持批量接口后改为批量查询，需求单 PROJ-1234 */
```

### 怎么写：精准 + 简洁

- 精准 = 命中业务对象与限定条件（标准同 R12），禁止"处理数据"、"执行操作"这类无信息措辞
- 简洁 = 默认一两行写完；长注释仅对复杂核心逻辑开放（分层标准同 R12）
- 格式 = 单行 `/* xxx */`，多行 `/* ... */`（同 R9）

### 寿命管理

- **改代码必须同步改注释**：过期注释比没有注释更糟——它会主动误导读者
- **禁止保留注释掉的死代码**：历史版本交给 git，不留"旧逻辑先注释掉"的残骸
- **TODO 必须有实质内容且可追踪**：主体说清要做什么、等什么条件；日期/负责人/关联单号只是辅助定位，不能替代内容
...
- 类/接口：说明职责与使用场景——什么时候用它、什么时候不用它
- 字段：仅当业务含义不直观时写——枚举值含义、单位、取值来源、特殊取值约定；含义直观的字段不写

```java
/**
 * 运单原始报文解析器
 * 仅处理承运商回传的原始报文；已标准化的轨迹数据走 TrackService，不经过本类
 */
public class RawTrackParser { ... }
```

```java
/** 可派件数（单位：件）；0 表示未评估，-1 表示该承运商已停用 */
private Integer dispatchableQty;
```

### 修复/业务分支注释：日期前缀

针对**已经存在的业务**做修复、兼容分支、业务特例分支、临时旁路、灰度补丁时，新代码部分的块注释建议以日期为前缀。

格式就一句话：`/* yyyy-MM-dd 现在做什么，解决什么问题 */`，问题与动作的顺序不限。

要点：

- **精准用词、简洁第一**：一句话能说清就不写两句；不需要类型标签（修复/兼容/特例），不堆砌机制细节
- **内容自包含**："做什么 + 解决什么问题"就是关键信息，读者看注释即懂，不用翻提交记录（见「引用不替代内容」）；自包含不等于长
- 日期前缀只是辅助定位，回答"这段特例什么时候来的"，配合 git blame 回溯背景
- 下线时机等补充信息仅在真正需要时用半句带上（如 `；老数据补齐后可下线`），不是必填

适用与不适用：

- 适用：bug 修复、老数据/老接口兼容分支、业务特例分支、临时旁路、灰度开关补丁
- 不适用：新业务的正常主线开发——主线代码不是补丁，不加日期前缀
- 若为修复新增了整个方法，Javadoc 首行可带同样前缀

正面示例（动作在前）：

```java
/* 2026-09-14 自清理过期标签，防止同步时一次读入撑爆堆 */
cleanExpiredTags();
```
...
```java
/* 2026-09-14 之前一次读入全量标签会 OOM，现在前置一步自清理 */
cleanExpiredTags();
```

反面示例（类型标签 + 细节堆砌，一句话写成流水账）：

```java
/* 2026-09-14 修复: 标签日志只增不删，同步时一次读入会撑爆堆；拼快照只要每个标签最新一条，写路径自清后可评估下线 */
cleanExpiredTags();
```
