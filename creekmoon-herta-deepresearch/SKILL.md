...
6. `Report Shaping`（固定必经）
   ...
   - 整形层必须剔除未确认项；研究边界只写覆盖范围与影响，不转抄待验证问题
...
- 不要把未确认内容伪装成结论
- 不要把未确认带进 `shaping-pack` 或 `final-report`，也不要改名后混入研究边界
...
- `45-report-shaping/shaping-pack.md` 已完成业务理解整形，并剔除未确认项
...
- [ ] `45-report-shaping/shaping-pack.md` 已完成全景、流转、生命周期、模块对接、设计原因与研究边界的规划
