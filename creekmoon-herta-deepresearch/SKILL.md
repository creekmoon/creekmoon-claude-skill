...
- 没有通过量化校验，不算完成
- 报告结构由用户这次的诉求意图决定，不套固定模板：先诊断处境与意图，再从用户问题推导目录
- 最终报告的首要目标是把用户的每个显式问题正面答到，且读起来不像 AI 套模板
- 报告不止逐条答问题，还要在证据支持下点亮用户没问但必须知道的判断（有纪律：限额 2 处、挂锚点、蓝图指定位置）
...
- `report-craft.md` 是报告审美与表达标准（标题安静、加粗克制、散文牵线、立场与洞察纪律）
...
6. `Report Blueprint`（固定必经，撰稿官第一步）
   ...
   - 目标：先诊断用户处境、判定报告意图（事实确认/决策支持/机制解释/业务理解/风险/对比/排查/混合），再从用户问题推导出贴合本次诉求的目录，写下核心洞见，规划按需图示
...
- 不要强行凑"最值得先记住的三件事"
- 不要写"安全报告"：只答被问的、句句均匀用力、看不出撰稿官认为什么最重要
- 不要写伪洞察：为显得有洞察而发明用户需求、夸大意外、未问但关键的点挂不出锚点
...
- `45-report-blueprint/report-blueprint.md` 已明确判定报告意图、映射用户问题、推导出本次专属目录、写下核心洞见，并剔除未确认项
...
- [ ] 蓝图 `Core Insight` 非空时已在正文点亮；"未问但关键"守纪律（≤ 2 处、挂锚点、标明超出所问）
- [ ] 报告判断有主次强弱，能看出撰稿官认为什么最重要（非句句均匀用力）
