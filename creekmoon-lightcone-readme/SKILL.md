...
这一节重点写 **代码里的核心对象** 如何被创建、增强、持久化、传递和消费。
...
| 对象 | 谁创建 | 谁补充 / 改变 | 谁持久化 | 谁最终消费 | 为什么重要 |
|------|--------|---------------|----------|------------|------------|

补充约束：

- 至少说明主对象与 `1-2` 个协作对象之间的关系，例如依附、触发、引用、派生
- 如果只是技术资源流转，可以作为补充视角，但不能替代核心对象流转
- 不要把对象表写成 DTO 字段说明；字段级细节留给二次提问或接口文档
...
- 关键对象只列名词，没有说明谁创建、谁改变、谁持久化、谁最终消费
...
- [ ] 有一条明确的代码主链路
- [ ] 主链路体现了先后顺序、谁生产、谁消费
- [ ] 核心对象之间的创建、改变、持久化、消费关系已经写清，不只是名词列表
- [ ] 有核心生命周期，而不只是某个局部流程
