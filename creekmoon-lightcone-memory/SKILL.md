...
│ 8. 扫描数据结构定义（DDL、ORM实体类、Schema定义、迁移文件）         │
...
| **实体边界** | 同一业务实体分散在哪些物理表？ | 用户实体分布在t_user/t_account/t_account_user |
| **字段来源** | 字段是原生存储还是计算得出？ | `total_amount` = `amount` + `tax` - `discount` |
| **术语歧义** | 不同模块对同一概念的不同叫法？ | "支付金额"vs"实付金额"vs"成交金额" |
