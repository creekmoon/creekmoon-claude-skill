...
> 这些关系往往蕴含更深层的业务耦合。
> 但请区分：关系存在 ≠ 已证明这是系统自有业务域；若只有映射或同步痕迹，应显式保守表述。
...
| 一公司一余额行 | `t_company` + `t_company_balance` | `BalanceQueryService#getByCompanyId()` 查询约束 | 查询返回 null | 后续操作 NPE |
```

## 待验证关系

| 候选关系 | 当前线索 | 为什么暂不下定论 | 后续验证方向 |
|----------|----------|------------------|--------------|
| {table_a} 与外部系统主数据的映射关系 | 命名 / DTO / 回调字段 | 只能证明存在对接痕迹，不能证明系统归属 | 继续检查主流程、核心表、状态机 |
